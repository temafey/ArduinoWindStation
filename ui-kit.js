...
// Прибор занимает всё, что осталось в блоке, а не отмеренные пиксели. Высота
// теперь свойство самого блока, и рисунок обязан идти за ней: иначе «сделать
// блок квадратным» давало бы квадрат с маленьким кольцом посередине.
//
// minHeight нужен для тех мест, где блока нет вовсе (модальные окна, вкладка
// радара): без него flex-ребёнок в обычном потоке схлопнулся бы в ноль.
export const INSTR_BOX = {
  flex: "1 1 auto", minHeight: INSTR_H / 2,
  display: "flex", alignItems: "center", justifyContent: "center",
  padding: 2,
};

// Пропорции сохраняются, а вписывается рисунок по высоте: у шкалы и картушки
// квадрат, у камеры прямоугольник, и совпасть они могут только по одной
// стороне. maxWidth обязателен — иначе квадрат вылезет за узкий блок.
