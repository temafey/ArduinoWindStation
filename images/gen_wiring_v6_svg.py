...
# сигнальная земля модема: CN101-7 «G» (кол.36) → «−» рельс
sig_gnd = to_rail(36, "b", RAIL_TM, SIGGND,
                  "CN101-7 «G» (кол.36) → «−» GND — сигнальная земля UART",
                  "4G · МОДЕМ",
...
HDR_PINS = [("G", "GND", SIGGND), ("R", "RXD", TX_C), ("T", "TXD", RX_C), ("K", "PWR", None),
...
power_wires = (
    ...
    mlead([(1350, GNDBUS), (1350, GND_RET), (X(41), GND_RET), H(41, "c")], GND,
          "земля блока → кол.41 = «G» GND модема (силовая)", "земля блока", "41c",
          "4G · ПИТАНИЕ", w=2.8))
...
NOTE_LINES = [
    ...
    ("#c0392b", '⚠  <tspan font-weight="700">Экран модуля смотрит ВНИЗ, на верхние рельсы.</tspan> В зоне свеса (кол. 32–46) на «+» рельсе не должно быть ни одной перемычки — там только «−» (35a, 36b, 44a, 45a), а земля с экраном и так одна цепь.'),
