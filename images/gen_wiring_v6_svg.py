...
отдельная карточка питания. v6 убирает всё висящее, что можно убрать:
  ...
  1. Гнездовая планка PLS-7 в ряду a, колонки 36–42 — модуль втыкается в неё штырями
     CN101. Свес уходит ВВЕРХ, за верхние рельсы и за край макетки; ряды b–e под ним
     остаются свободными, и вся обвязка модема живёт там же, в тех же колонках.
  ...
  3. C4 1000 мкФ переехал на макетку, в колонки 40/41 — это буквально два ряда
     от контакта VCC, ближе к модему электролит уже не поставить.
  4. C1 1000 мкФ уехал с 39/41 на 43/44: колонки 36–42 забрала планка.
  5. С платы уходят ВСЕГО ЧЕТЫРЕ провода: 49j → ключ → B3 IN+, 45i → земля блока,
     B3 OUT+ → кол.40, B3 OUT− → кол.41. Всё остальное — перемычки на макетке.
...
caps_elec = (bb.cap_electrolytic(40, 41, "e", "C4") +
...
# сигнальная земля модема: CN101-7 (кол.36) → «−» рельс
sig_gnd = to_rail(36, "b", RAIL_TM, SIGGND,
                  "CN101-7 (кол.36) → «−» GND — сигнальная земля UART",
                  "4G · МОДЕМ",
...
# UART: с верхних пинов ESP32 прямо в колонки планки, ряд c
uart_wires = (
    jmp([(bb.PIN_TOP["27"], 311), (bb.PIN_TOP["27"], UART_LANE["tx"]),
         (X(37), UART_LANE["tx"]), H(37, "c")], TX_C,
        "GPIO27 (TX) → кол.37 = CN101-6 RXD", "GPIO27", "37c", "4G · МОДЕМ", w=2.4,
    ...
    jmp([(bb.PIN_TOP["26"], 311), (bb.PIN_TOP["26"], UART_LANE["rx"]),
        ...
        "GPIO26 (RX) ← кол.38 = CN101-5 TXD", "GPIO26", "38c", "4G · МОДЕМ", w=2.4,
...
# ═══════════════════════════════════════ 13. ПЛАНКА + СВЕС МОДЕМА НА МАКЕТКЕ
HDR_L, HDR_R = 36, 42
# контакты CN101 идут СПРАВА НАЛЕВО: свес обязан уходить вверх, за край макетки
HDR_PINS = [(7, "GND", SIGGND), (6, "RXD", TX_C), (5, "TXD", RX_C), (4, "PWR", None),
            (3, "VCC", PLUS), (2, "GND", GND), (1, "SLP", None)]
header = bb.header_socket(HDR_L, HDR_R, "a", HDR_PINS)

OVW = 248                                        # 37 мм в масштабе макетки
OVX = (X(HDR_L) + X(HDR_R)) / 2 - OVW / 2
overlay = bb.overlay_outline(
    OVX, 230, OVW, 46,
    "BK-A7670 37×37 мм — свес ВВЕРХ, за край макетки")
...
FETX, FETY, FETW, FETH = 1266, 268, 116, 56
...
power_panel = (
    ...
    f'⚠ 1.5 А идут через защиту DW01/FS8205: ~50 мОм → 75 мВ и 0.11 Вт, порог OCP ~3 А — проходит</text>')
...
power_wires = (
    ...
    # возврат на макетку — два провода в колонки планки
    mlead([(B3_OUT_P, B3Y), (B3_OUT_P, VCC_RET), (X(40), VCC_RET), H(40, "c")], PLUS,
          "Boost#3 OUT+ (5.2 В) → кол.40 = CN101-3 VCC", "B3 OUT+", "40c", "4G · ПИТАНИЕ",
          w=2.8, note="в этой же колонке сидит C4 — электролит стоит у самого модема") +
    mlead([(1350, GNDBUS), (1350, GND_RET), (X(41), GND_RET), H(41, "c")], GND,
          "земля блока → кол.41 = CN101-2 GND (силовая)", "земля блока", "41c",
          "4G · ПИТАНИЕ", w=2.8))
...
PBX, PBY, PBW, PBH = 1090, 620, 640, 340
...
MAPTXT = ("7 GND → кол.36 · 6 RXD → кол.37 · 5 TXD → кол.38 · "
          "4 PWRKEY → кол.39 (н/п) · 3 VCC → кол.40 · 2 GND → кол.41 · 1 SLEEP → кол.42 (н/п)")
...
modem_panel = (
    ...
    f'4G/GPS-модуль — садится в планку кол. 36–42, проводов к нему НЕТ</text>' +
    bb.mod_a7670(MODX, MODY, MODW, MODH, used=CN_USED) + antennas +
    f'<text x="{PBX+18}" y="{PBY+PBH-42}" font-size="9.5" font-weight="700" fill="#2a6fd1">{MAPTXT}</text>'
    ...
    f'⚠ плата накрывает только рельсы (ряды b–e свободны) и висит 27 мм за краем макетки — '
    f'стойка M2 в монтажное отверстие дальней кромки обязательна</text>')
...
TOPC = [19, 23, 25, 27, 29, 31, 33, 35, 38, 40, 43]
...
title = '''<text x="30" y="38" font-size="25" font-weight="700" fill="#1a1a1a">Схема v6 · «Компакт-1» — 4G/GPS-модуль стоит прямо на макетке</text>
  <text x="30" y="62" font-size="14" fill="#666">Модем садится в гнездовую планку (кол. 36–42, ряд a). С платы уходят всего четыре провода — два в буст №3 и два обратно. Диодное ИЛИ модема снято, C1 переехал на 43/44.</text>'''

rules = '''<rect x="30" y="70" width="1180" height="78" rx="8" fill="#f6f8fb" stroke="#ccd6e4"/>
  <text x="46" y="91" font-size="12.5" fill="#1a1a1a"><tspan font-weight="700">Планка только на 36–42.</tspan> Левее нельзя: <tspan font-weight="700">35a — низ делителя датчика</tspan>, то есть земля; контакт RXD сел бы прямо на неё. Свес уходит вверх и накрывает рельсы — ряды b–e остаются рабочими.</text>
  <text x="46" y="109" font-size="12.5" fill="#1a1a1a"><tspan font-weight="700">Один источник вместо диодного ИЛИ:</tspan> буст №3 кормится с <tspan font-weight="700">OUT+ TP4056</tspan>. Цена решения — <tspan font-weight="700">без установленной банки модем не заведётся</tspan>: TP4056 в одиночку 1.5 А не отдаст.</text>
  <text x="46" y="127" font-size="12.5" fill="#1a1a1a"><tspan font-weight="700">UART без переворота:</tspan> GPIO27 (TX) → кол.37 = RXD, GPIO26 (RX) ← кол.38 = TXD. <tspan font-weight="700">Serial2 по умолчанию сидит на GPIO16/17 — это жёлтый и зелёный светодиоды</tspan>, пины задавать явно.</text>
...
NY = int(table_end) + 16
NOTE_LINES = [
    ("#1a1a1a", '<tspan font-weight="700">Что ушло по сравнению с v5.</tspan> Два Шоттки 3 А (SS34/1N5822), адаптер на 3 А, отдельная карточка диодного ИЛИ и пять проводов к висящему модему. Осталась одна карточка и четыре провода.'),
    ...
    ("#1a1a1a", '<tspan font-weight="700">Почему планка именно на 36–42.</tspan> Колонка 35 — низ делителя датчика (сидит на «−» рельсе): контакт RXD ушёл бы в землю. Колонки 43–44 забрал переехавший C1, 45 и правее — под корпусом TP4056.'),
    ("#c0392b", '⚠  <tspan font-weight="700">Механика: 27 мм консоли на семи штырях.</tspan> От ряда a до края макетки ~10 мм, остальные 27 мм платы 37×37 висят в воздухе — вместе с SIM-держателем и двумя косичками u.FL, которые тянут.'),
    ("#333",    'Лечится штатно: на дальней кромке платы есть два монтажных отверстия — одна стойка M2 в дно корпуса снимает весь момент с гребёнки. Без стойки пружины планки разбиваются за пару снятий модуля.'),
    ("#c0392b", '⚠  <tspan font-weight="700">Четыре «слепых» провода ставятся ДО посадки модема:</tspan> 35a, 36b, 44a и 45a — все идут на верхний «−» рельс, который потом закрыт свесом платы. Забыл — снимать модуль.'),
    ("#333",    '<tspan font-weight="700">Два электролита ⌀10 мм стоят в 7.6 мм друг от друга</tspan> (C4 на 40/41, C1 на 43/44) — банки надо развести наклоном в стороны, а C1 дополнительно отклонить ВЛЕВО от корпуса TP4056.'),
    ("#333",    '<tspan font-weight="700">C4 — ровно там, где нужен.</tspan> Два ряда от контакта VCC: бросок тока на старте передачи гасится в той самой точке, где возникает. Полярность — «+» в колонку 40. Номинал 1000 мкФ, напряжение ≥10 В.'),
    ("#8a6a1a", '◌  <tspan font-weight="700">Сигнальная земля (36b → рельс) — по желанию.</tspan> Она даёт UART короткий обратный путь, но параллелит силовой: по рельсу пойдёт ~20–30% пика, то есть 0.3–0.45 А — в пределах клипсы MB-102, но не ноль.'),
    ("#8a6a1a", 'Не хочется делить ток — перемычку можно просто не ставить: опору модем всё равно получит через силовую землю на кол.41. Обратного эффекта нет, UART станет чуть менее устойчив к наводке.'),
