...
def module_pin_row(col_l, col_r, row, pins, title="", title_dx=-14):
    """Ряд контактов модуля, который воткнут штырями ПРЯМО в один ряд макетки —
       ...
       пишется коротко (VCC, RXD), а в самом контакте — то, чем его опознают на плате:
       буква шелкографии, если она есть, иначе номер. pins слева направо:
       (буква или номер, короткая метка, цвет или None); None = контакт никуда не разведён.
       Порядок в списке = порядок НА МАКЕТКЕ, а он зависит от того, какой стороной модуль
       лёг: считать по фотографии платы, а не по картинке из мануала."""
    y = ROWY[row]
