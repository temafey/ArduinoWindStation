...
**Как открыть дашборд:** подключиться к сети `WindStation` (пароль `<AP-пароль>`) и открыть `http://MyWindProbeBETA.org` — или `http://192.168.4.1`, если браузер капризничает. Обычно портал всплывает сам сразу после подключения.

**Датчик:** Polycarbonate 0-5V Wind Speed & Direction Sensor (с AliExpress)
**Диапазон:** скорость 0–60 м/с, направление 0–360°
**Питание датчика:** 10–30V DC (используем 12V)
**Выходной сигнал:** аналоговый 0–5V
...
| 2 | Датчик ветра Polycarbonate 0-5V | Скорость + направление (уже заказан) |
...
| 8 | Резисторы 10kΩ ×4 | Сигнальный делитель: **нижнее** плечо (×2) + **верхнее** плечо, 1-й резистор в серии (×2) |
| 9 | Резисторы 5kΩ ×2 | **Верхнее** плечо сигнального делителя, 2-й резистор в серии (10k+5k = 15k; 5V → 2.0V, чистая линейная зона ADC). Отдельного 15kΩ в наборе нет |
| 10 | Резисторы 100kΩ ×2 | Делитель для мониторинга напряжения батареи на GPIO32 |
| 11 | Резисторы 220Ω ×5 | Токоограничение для LED |
| 12 | Керамический конденсатор 100 нФ ×4 | Фильтры ADC входов (×2), VIN (×1), батарейный делитель (×1) |
...
Ничего из этого блока в станции пока не стоит. Список отделён намеренно: расчёты, из которых
он получен, лежат в `tasks/10-a7670e-4g-gps.md`, §Питание — **перед закупкой прочитать
именно их**, потому что три позиции здесь заменяют то, что уже стоит в станции, а не
добавляются к нему.

| # | Компонент | Назначение и почему именно такой |
|---|---|---|
| 26 | Плата BK-A7670 V1 (SIMCom A7670E) + плёночная LTE-антенна + патч GPS 1575 МГц | 4G-аплинк и координаты. **Уже на руках**, антенны подключены к J1/J2. Гребёнка CN101 распаяна — паять не нужно |
| 27 | Mini Boost MT3608 **№3** | Отдельный источник 5.2 В для VCC модуля. Шина станции 4.7 В ниже минимума платы (5 В), буст №1 на 12 В — выше максимума |
| 28 | Диод Шоттки **3 А**: SS34 / 1N5822 / SR340 ×**2** | Собственное диодное ИЛИ для входа буста №3. ⚠️ **1N5819 сюда нельзя** — у него предел 1.0 А, а через диод пойдёт до 1.5 А сеансами по 10 с |
| 29 | Электролит 1000 мкФ 16 В ×1 (второй в станции) | На VCC у самой платы модуля. Штатный тантал 470 мкФ на плате стоит **после** линейного стабилизатора и входную просадку не держит |
| 30 | PTC-предохранитель **3 А** radial (замена позиции 14b) | Пик на разряженной банке ~1.83 А: модем 1.50 А + станция 0.33 А. Двухамперный работал бы на границе срабатывания — а его в июле уже вышибало |
| 31 | Адаптер 5 В **3 А** — **либо** резистор Rprog 2.4 кΩ на TP4056 | Случай «модем передаёт, пока батарея заряжается» даёт 2.01 А на текущем двухамперном адаптере (позиция 23). Или адаптер помощнее, или ток заряда вдвое меньше. **Выбрать до сборки** |
| 32 | P-канальный MOSFET (high-side ключ) + резистор затвора, на GPIO25 | Снятие питания с модема. PWRKEY прижат к земле через R104, а выход из CMUX у A7670 сломан — повисший модем поднять больше нечем. **Для станции у розетки необязательно, для мачты — условие переезда** |
...
flowchart TB
    ...
    SENSOR[Датчик ветра<br/>выход 0-5V]
    DIV1[Делитель 10k+5k / 10k<br/>канал скорости + 100нФ]
    DIV2[Делитель 10k+5k / 10k<br/>канал направления + 100нФ]
...
| GPIO35 (ADC1) | Выход делителя (10k+5k)/10k — направление | Чтение направления ветра (0–2.0V) |
...
flowchart TB
    subgraph BOX["КОРПУС IP65  190 × 150 × 75 мм  крышка глухая"]
        ...
        subgraph TOP["Верхний уровень — breadboard 165×55"]
            ...
            DIV["Делители напряжения<br/>резисторы 10k+5k / 10k"]
...
Нужно **два таких делителя** — один для скорости (GPIO34), один для направления (GPIO35). Конденсатор 100 нФ на каждом выходе делителя между GPIO и GND — фильтр высокочастотных наводок с длинного кабеля от мачты.
...
| Домашние сети | список `homeNetworks[]` в прошивке; сейчас в нём одна — `HomeSSID` |
...
- **Сетей может быть сколько нужно.** `homeNetworks[]` — список пар «имя, пароль», порядок
  задаёт приоритет: дом, дача, телефон-раздача. Плата пробует их по очереди, по 12 секунд
...
- **Если ни одной из сетей рядом нет** (мачта, дача, поле) — плата пробует всё реже:
  ...
  Совсем отключить попытки — поставить `HAS_HOME_NETWORK 0` в прошивке
...
- Пароль OTA: `<OTA-пароль>` (переменная `otaPassword` в прошивке)
...
4. Собери два сигнальных делителя: сверху **10k + 5k последовательно (= 15k)**, снизу **10k** (для скорости и направления)
5. На выход каждого делителя (перед GPIO34/GPIO35) поставь **100 нФ** на землю — фильтр шумов
6. Подключи выходы делителей к GPIO34 (скорость) и GPIO35 (направление)
...
5. Ничего про WiFi вписывать не нужно — станция поднимает собственную сеть, параметры вшиты
   в константы `apSsid` / `apPassword`. По желанию их и `otaPassword` можно поменять там же
...
9. Подключи телефон или ПК к сети `WindStation` (пароль `<AP-пароль>`) и открой
   `http://MyWindProbeBETA.org`. Обычно портал всплывает сам
10. При отсутствии датчика LED `ОШИБКА` (GPIO18) загорится — прошивка детектирует «висящие»
    ADC пины. Это нормально
...
2. Подключи провода датчика по схеме (цвета уточни в инструкции датчика):
   ...
   - Сигнал скорости → вход делителя 1
   - Сигнал направления → вход делителя 2
3. Подай питание
4. Проверь что датчик вращается свободно (подуй на чашки)
5. Открой веб-дашборд — должны появиться реальные значения скорости и направления
...
2. Подключи приставку к домашней сети — станция там тоже есть, и интернет у приставки
   останется. Если дома роутера нет, подключай к `WindStation` (пароль `<AP-пароль>`), но учти:
...
- [ ] Керамика 100 нФ стоит у VIN ESP32 и на выходах ADC-делителей
- [ ] Сигнальные делители собраны: сверху **10k + 5k последовательно (= 15k)** (со стороны сигнала), **10k** снизу (к GND)
...
- [ ] Сеть `WindStation` видна с телефона и пускает по паролю `<AP-пароль>`
...
| Дашборд показывает «ADC · ОШИБКА» | Датчик не подключён или обрыв | Прошивка детектит висящие пины, проверь проводку |
