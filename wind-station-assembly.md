...
именно их**. Позиция 30 не добавляется, а **заменяет** уже стоящее (PPTC вместо позиции 14b),
и список успел сократиться: две позиции выпали при переходе на компоновку v6, см. врезку
под таблицей.
...
| 28 | Гнездовая планка PLS «мама», **7 контактов**, шаг 2.54 | Ряд `a`, колонки 36–42: модуль садится в неё штырями CN101 прямо на макетку, dupont-провода к модему не нужны вовсе. Можно отрезать от длинной планки |
| 29 | Электролит 1000 мкФ **≥10 В** ×1 (второй в станции) | C4 — в колонки 40/41 макетки, два ряда от контакта VCC модема. Штатный тантал 470 мкФ на плате стоит **после** линейного стабилизатора и входную просадку не держит. Корпус ⌀8 мм удобнее ⌀10: рядом стоит C1 |
...
| 31 | Стойка **M2** + винт | Под дальнюю кромку платы модема: 27 мм её висят за краем макетки на семи штырях, вместе с SIM и двумя косичками u.FL. На дальней кромке BK-A7670 два монтажных отверстия |
| 32 | P-канальный MOSFET (high-side ключ) + резистор затвора, на GPIO25 | Снятие питания с модема. PWRKEY прижат к земле через R104, а выход из CMUX у A7670 сломан — повисший модем поднять больше нечем. **Для станции у розетки необязательно, для мачты — условие переезда.** В v6 у него вторая работа: между сеансами он снимает нагрузку с зарядного узла |

> **Как это подключается:** разводка блока нарисована в **`wiring-v6.html`** («Компакт-1»:
> модуль стоит прямо на макетке в планке кол. 36–42, буст №3 кормится только с `OUT+` TP4056,
> UART на GPIO27/26 приходит в колонки 37/38). С макетки уходят четыре провода — `49j`, `45i`
> и два обратно в колонки 40/41. Единственная правка существующей платы: **C1 переезжает
> с 39/41 на 43/44**. Предыдущий вариант (модем на весу, своё диодное ИЛИ на двух Шоттки 3 А)
> лежит в `wiring-v5.html` и к исполнению не берётся.
>
> **Что выпало из этого списка при переходе на v6:** два Шоттки 3 А (SS34 / 1N5822 / SR340)
> и адаптер 5 В 3 А либо Rprog 2.4 кΩ. Диодного ИЛИ у модема больше нет, а заряд идёт своим
> током — штатного адаптера 2 А (позиция 23) хватает. Цена решения: **без установленной банки
> модем не заводится**, TP4056 в одиночку пик 1.5 А не отдаст. Разбор — в
