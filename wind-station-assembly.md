...
| Домашняя сеть | `HomeSSID` (константы `staSsid` / `staPassword` в прошивке) |
| Адрес в домашней сети | выдаёт роутер; смотреть поле `staIp` в `/api/data` или строку в Serial |

**Про домашнюю сеть.** Вписывать её через дашборд нельзя и не задумано — это две константы
в скетче, меняются вместе с перепрошивкой. Три вещи, которые важно знать:
...
- **Если домашней сети рядом нет** (мачта, дача, поле) — плата пробует подключиться всё реже:
  30 секунд, минута, две… до десяти минут. Так сделано потому, что каждая попытка уводит радио
  с канала своей сети, и дашборд на это время замирает. Совсем отключить попытки — поставить
  `HAS_HOME_NETWORK 0` в прошивке
