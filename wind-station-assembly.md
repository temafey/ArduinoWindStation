...
> **Как это подключается:** разводка блока нарисована в `wiring-v5.html` (v4 плюс модем справа
> от макетки — своё диодное ИЛИ, третий буст 5.2 В, электролит у платы модуля, UART на GPIO27/26).
> Плата v4 при этом не меняется ни одной перемычкой: макетка отдаёт блоку три точки — `37j`, `49j`, `45i`.
>
