...
| 15 | LED зелёный 5мм ×2 | Заряд батареи >60% + WiFi (мигает при подключении) |
...
| 21 | Корпус IP65 190×150×75 мм (прозрачная крышка) | Защита от влаги |
...
| GPIO5  | 220Ω (на плате) → анод LED на крышке | Зелёный — WiFi: мигает пока подключается, горит когда подключён (strapping-пин, кратко мигает при загрузке) |
...
```mermaid
flowchart TB
    subgraph BOX["КОРПУС IP65  190 × 150 × 75 мм  прозрачная крышка"]
        direction TB
        subgraph TOP["Верхний уровень — breadboard 165×55"]
            ESP32["ESP32 DevKit V1"]
            DIV["Делители напряжения<br/>резисторы 10k+5k / 10k"]
            LEDROW["5 LED у прозрачной стенки<br/>на проводах dupont"]
        end
...
**Размещение светодиодов:** все 5 LED выносятся длинными проводами dupont м-ж к прозрачной крышке или стенке корпуса — чтобы статус станции был виден снаружи без открытия.
...
**Станция — это точка доступа, и только она.** В домашнюю сеть плата не заходит: не сканирует,
не подключается, свою сеть не выключает. Так сделано намеренно — в прежней версии AP жила лишь
пока не найдена знакомая сеть, и через ~15 секунд после включения станция просто пропадала
с телефона.
...
**mDNS** тоже работает, как запасной путь: `http://mywindprobebeta.local`.

**OTA (обновление по воздуху):**
- Подключить компьютер к сети `WindStation` — заливать можно только оттуда, другого пути в плату нет
- Arduino IDE → `Инструменты → Порт → Network Port` → `mywindprobebeta at 192.168.4.1`
...
5. Впиши свой WiFi SSID и пароль в константы `ssid` и `password`
6. По желанию: поменяй `hostname` (по умолчанию `windstation`) и `otaPassword`
7. Выбери плату: **ESP32 Dev Module**, правильный COM-порт
8. Нажми Upload
9. Открой Serial Monitor (115200 бод) — увидишь IP-адрес и строку `mDNS: http://windstation.local`
10. Открой `http://windstation.local` в браузере телефона или ПК (или IP, если mDNS не резолвится)
11. При отсутствии датчика LED `ОШИБКА` (GPIO18) загорится — прошивка детектирует «висящие» ADC пины. Это нормально.
12. После первой USB-прошивки дальнейшие обновления можно делать по OTA: Arduino IDE → `Инструменты → Порт → Network Port → windstation`
...
7. Выведи светодиоды проводами к прозрачной крышке
...
2. Пропиши в настройках URL: `http://windstation.local` (или IP ESP32, если Android TV не резолвит mDNS)
3. Если `windstation.local` не работает на Android TV — используй IP (смотри в Serial Monitor или роутере)
...
- [ ] WiFi SSID и пароль прописаны в скетче
- [ ] ESP32 получил IP и отвечает в браузере (по `windstation.local` или IP)
...
| WiFi не подключается | Неверный SSID/пароль | Исправь в скетче, перепрошей |
| `windstation.local` не резолвится | mDNS не работает на устройстве | Установи Bonjour (Windows), используй IP напрямую |
...
- ~~**WiFiManager / Captive Portal** — настройка SSID/пароля без перепрошивки~~ — ✅ сделано иначе (2026-08-02): до 6 сетей в NVS, управление из дашборда (`/api/wifi`), при отсутствии сетей — своя AP `WindStation-Setup` с дашбордом на 192.168.4.1
...
- Красивый дашборд в реальном времени на большом мониторе (доступен по `windstation.local`)
