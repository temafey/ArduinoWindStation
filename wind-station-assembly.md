...
2. Подключи приставку к WiFi-сети `WindStation` (пароль `<AP-пароль>`). Учти: пока она в этой
   сети, интернета у неё нет — у станции нет аплинка
3. Пропиши в настройках URL: `http://192.168.4.1`. Для киоска лучше именно IP: он не зависит
   ни от DNS, ни от того, как прошивка Fully обрабатывает переход с HTTPS на HTTP
