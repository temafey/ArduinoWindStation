...
|---|---|---|---|
| 26 | Плата BK-A7670 V1 (SIMCom A7670E) + плёночная LTE-антенна + патч GPS 1575 МГц | ✅ на руках | 4G-аплинк и координаты. Антенны подключены к J1/J2. Гребёнка CN101 распаяна — паять не нужно. ⚠️ **Штыри торчат со стороны наклейки**, поэтому на макетку модуль ложится наклейкой вниз |
...
| 29 | Электролит 1000 мкФ **≥10 В** ×1 | ✅ есть | C4 — в колонки 37/38 макетки, два ряда от контакта VCC модема. **Та же деталь, что позиция 13** (C1 на шине). Штатный тантал 470 мкФ на плате стоит **после** линейного стабилизатора и входную просадку не держит. Корпус ⌀8 мм удобнее ⌀10: рядом стоит C1 |
...
> который тянет затвор к земле по GPIO25 — напрямую с 3.3 В логики верхний ключ не закрывается.
> Логика при этом инвертируется: **GPIO25 HIGH = модем включён**.
...
> UART на GPIO27/26 приходит в колонки 41/40). С макетки уходят четыре провода — `49j`, `45i`
> и два обратно в колонки 38/37. Единственная правка существующей платы: **C1 переезжает
...
>
> ⚠️ **Нумерация CN101 на макетке зеркальна мануалу.** Модуль лежит наклейкой вниз, поэтому
> кол.36 = контакт 1 (SLEEP), 37 = 2 GND, 38 = 3 VCC, 39 = 4 PWRKEY, 40 = 5 TXD, 41 = 6 RXD,
> 42 = 7 GND. Собранный «по мануалу» узел подаёт 5.2 В на TXD модуля. Сверяться только со схемой.
>
